...
public class SQLQuery {
        ...
        public static final String searchServiceQuery = "select s.*, v.company_street, v.company_city, v.company_province, v.company_country, avg(r.rating) as avgRating, count(distinct b.booking_id) as totalBookings from services as s left join service_category_association as sca on s.service_id = sca.service_id left join service_categories as c on sca.category_id = c.category_id left join vendors as v on s.user_id = v.user_id left join reviews as r on s.service_id = r.service_id left join bookings as b on s.service_id = b.service_id where service_name like concat('%', ?, '%') or service_description like concat('%', ?, '%') or service_price like concat('%', ?, '%') or c.category_name like concat('%', ?, '%') or c.category_description like concat('%', ?, '%') group by s.service_id order by totalBookings desc, s.service_id;";
